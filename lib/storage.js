...
// Atomic read-modify-write: runs INSIDE the same per-file queue as writeJSON,
// so concurrent callers (e.g. the campaign send loop and the Baileys reply
// listener both mutating contacts.json) never race on a stale read. Plain
// readJSON()+writeJSON() from two independent callers is NOT atomic — each
// reads synchronously before the other's write lands, and the second write
// silently discards the first's change (a lost update). mutatorFn receives
// the current value and returns the new value.
function mutateJSON(name, mutatorFn, defaultValue) {
  const filePath = jsonPath(name);
  return queueWrite(filePath, async () => {
    let current;
    try {
      ...
      if (err.code === 'ENOENT') current = defaultValue;
      else throw err;
    }
...
module.exports = { readJSON, writeJSON, mutateJSON, appendJSONL, readJSONL, DATA_DIR };
