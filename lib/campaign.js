...
class CampaignEngine {
  ...
  async _sendToContacts(contacts, state) {
    ...
    for (const contact of contacts) {
      ...
      const variant = pickVariant(variants, fresh.nextVariantIndex);
      if (!variant) {
      ...
      try {
        ...
        fresh.nextVariantIndex = (fresh.nextVariantIndex + 1) % variants.length;
        ...
        if (fresh.consecutiveFailures >= 5) {
          fresh.status = 'paused';
          fresh.pauseReason = 'consecutive_failures';
          await this.saveState(fresh);
          return;
        }
        await this.saveState(fresh);
