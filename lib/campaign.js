...
// No per-contact name data — strip any stray {{name}} placeholder rather
// than send literal template syntax to a real contact.
function personalize(text) {
  return text.replace(/\{\{\s*name\s*\}\}/gi, '').replace(/ {2,}/g, ' ').trim();
}
...
// Splits a WhatsApp JID into its id and whether it's a LID (privacy
// identifier, "<id>@lid") rather than a phone-number JID
// ("<phone>@s.whatsapp.net"). Verified live: a real reply arrived as
// "185375200931933@lid", not the sender's actual phone number.
function parseJid(jid) {
  const [id, domain] = String(jid).split('@');
  return { id, isLid: domain === 'lid' };
}
...
class CampaignEngine {
  ...
  // Full reset for a fresh campaign with the same contact list: every
  // contact goes back to Pending (see contactsLib.resetAll), and campaign
  // progress (status, current batch, variant rotation, failure counter)
  // goes back to defaults. Pacing settings are kept — resetting your
  // contact list shouldn't also throw away tuned batch size/delays/quiet
  // hours. The dashboard downloads a report before calling this, since
  // it erases the record of who replied.
  async resetCampaign() {
    await contactsLib.resetAll();
    return this.updateState((state) => ({ ...defaultState(), config: state.config }));
  }
  ...
  async _onReply({ from }) {
    const { id, isLid } = parseJid(from);
    const number = isLid ? contactsLib.findNumberByLid(id) : id;
    if (!number) {
      // A LID we've never seen before (e.g. a contact messaged before LID
      // tracking existed) can't be resolved automatically — log it clearly
      // rather than silently dropping the reply, so it's at least visible.
      await this.log({ type: 'unresolved_reply', from, reason: 'lid not mapped to a known contact' });
      return;
    }
  ...
  async _maybeStartBatch(state) {
    ...
    const candidates = nextBatchContacts(contacts, state.config.batchSize);
    ...
    // Numbers that are syntactically valid but not registered on WhatsApp
    // get sorted into their own group here, before they ever become part
    // of a batch — one bad number should not burn a send attempt or eat
    // into the consecutive-failure counter every campaign run.
    const checks = await this.baileys.checkOnWhatsApp(candidates.map((c) => c.number));
    // Learn each contact's LID here too — some replies arrive addressed by
    // LID instead of phone number (verified live), and this check is the
    // one place Baileys hands back that pairing.
    await contactsLib.setLids(checks);
    const invalidNumbers = checks.filter((r) => !r.exists).map((r) => r.number);
    if (invalidNumbers.length) {
      await contactsLib.markInvalid(invalidNumbers);
      await this.log({ type: 'invalid_number', numbers: invalidNumbers });
    }
    const nextContacts = candidates.filter((c) => !invalidNumbers.includes(c.number));
    if (nextContacts.length === 0) return; // next tick pulls a fresh (now-smaller) Pending set
  ...
  async _sendToContacts(contacts, state) {
    ...
    for (const contact of contacts) {
      ...
      const text = personalize(variant.text);
