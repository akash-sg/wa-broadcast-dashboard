...
class BaileysConnection extends EventEmitter {
  ...
  // it left out, matching order 1:1 with the input. Also returns each
  // contact's LID (WhatsApp's newer privacy-preserving identifier) — some
  // replies arrive addressed by LID instead of phone number (verified live:
  // a real reply showed up as remoteJid "185375200931933@lid", not the
  // sender's actual number), and this USync response is the one place
  // Baileys hands back the LID<->phone-number pairing so campaign.js can
  // resolve those replies back to the right contact.
  async checkOnWhatsApp(numbers) {
    if (this.status !== 'connected') throw new Error('not connected');
    const results = await this.sock.onWhatsApp(...numbers.map(toJID));
    const byDigits = new Map(
      (results || []).map((r) => [String(r.jid).replace(/\D/g, ''), r])
    );
    return numbers.map((number) => {
      const match = byDigits.get(String(number).replace(/\D/g, ''));
      return {
        number,
        exists: Boolean(match),
        lid: match?.lid ? String(match.lid).replace(/\D/g, '') : null,
      };
    });
  }
}
