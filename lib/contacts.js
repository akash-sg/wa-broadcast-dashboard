    ...
    for (const row of dataRows) {
      ...
      if (!byNumber.has(number)) {
        byNumber.set(number, {
          ...
          sentAt: null,
          repliedAt: null,
          lid: null, // WhatsApp's privacy-ID for this contact, learned at send time
        });
      }
...
// Records each contact's LID (WhatsApp's privacy-preserving identifier),
// learned from the onWhatsApp check that already runs before every batch.
// Needed because some replies arrive addressed by LID instead of phone
// number — findNumberByLid is how those get resolved back to a contact.
async function setLids(pairs) {
  const byNumber = new Map(pairs.filter((p) => p.lid).map((p) => [p.number, p.lid]));
  if (byNumber.size === 0) return getContacts();
  return storage.mutateJSON(CONTACTS_KEY, (contacts) => {
    for (const c of contacts) {
      const lid = byNumber.get(c.number);
      if (lid) c.lid = lid;
    }
    return contacts;
  }, []);
...
// A reply/reaction whose remoteJid ends in @lid instead of @s.whatsapp.net
// can only be matched to a contact if we previously learned their LID via
// setLids. Returns null if unknown — that reply can't be attributed
// automatically (this is exactly what happens for a contact messaged
// before this LID-tracking existed; nothing to do about it retroactively
// except manually confirm and mark them, which is a one-off, not a bug).
function findNumberByLid(lid) {
  const contact = getContacts().find((c) => c.lid === lid);
